...
A join token can be issued to allow a new cluster member with a given name to join the cluster. A join token is a `base64` encoded string that must be supplied with a join request.

Join tokens can only be issued over the local unix socket of an initialized cluster member. To issue a token, a cluster member name and token expiry must be provided. The given name must be present in the `server.crt` Subject Alternative Names of the joining cluster member.
