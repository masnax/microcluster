...
// StartWithPeers starts up dqlite and joins the cluster of peers.
func (db *DB) StartWithPeers(peers map[string]types.AddrPorts, clusterCert *shared.CertInfo, listenAddr api.URL) error {
	allPeerAddrs := []string{}
	for _, peerAddrs := range peers {
		allPeerAddrs = append(allPeerAddrs, peerAddrs.Strings()...)
	}
	...
	db.dqlite, err = dqlite.New(db.dir,
		dqlite.WithAddress(listenAddr.URL.Host),
		dqlite.WithCluster(allPeerAddrs),
		dqlite.WithExternalConn(db.dialFunc(), db.acceptCh),
		dqlite.WithUnixSocket(os.Getenv(sys.DqliteSocket)))
	if err != nil {
		return fmt.Errorf("Failed to bootstrap dqlite: %w", err)
...
func dqliteNetworkDial(ctx context.Context, addr string, db *DB) (net.Conn, error) {
	...
	path := fmt.Sprintf("https://%s%s", addr, "/restricted/database")
